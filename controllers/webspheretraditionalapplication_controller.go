...
const indexFieldImageStreamName = "spec.applicationImage"
...
func (r *ReconcileWebsphereTraditional) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		reqLogger.Error(err, "Error validating WebsphereTraditionalApplication")
	...
	err = r.GetClient().Update(context.TODO(), instance)
	if err != nil {
		reqLogger.Error(err, "Error updating WebsphereTraditionalApplication")
	...
	if imageReferenceOld != instance.Status.ImageReference {
		...
		if err != nil {
			reqLogger.Error(err, "Error updating WebsphereTraditional status")
			return r.ManageError(err, common.StatusConditionTypeReconciled, instance)
		}
	}
	...
	reqLogger.Info("Reconcile WebsphereTraditionalApplication - completed")
...
func (r *ReconcileWebsphereTraditional) SetupWithManager(mgr ctrl.Manager) error {
	...
	mgr.GetFieldIndexer().IndexField(context.Background(), &webspheretraditionalv1alpha1.WebsphereTraditionalApplication{}, indexFieldImageStreamName, func(obj client.Object) []string {
		instance := obj.(*webspheretraditionalv1alpha1.WebsphereTraditionalApplication)
		image, err := imageutil.ParseDockerImageReference(instance.Spec.ApplicationImage)
		if err == nil {
	...
	b := ctrl.NewControllerManagedBy(mgr).For(&webspheretraditionalv1alpha1.WebsphereTraditionalApplication{}, builder.WithPredicates(pred)).
...
func (r *ReconcileWebsphereTraditional) finalizeWebsphereTraditionalApplication(reqLogger logr.Logger, wtapp *webspheretraditionalv1alpha1.WebsphereTraditionalApplication, pvcName string, pvcNamespace string) error {
	r.deletePVC(reqLogger, pvcName, pvcNamespace)
	return nil
}

func (r *ReconcileWebsphereTraditional) addFinalizer(reqLogger logr.Logger, wtapp *webspheretraditionalv1alpha1.WebsphereTraditionalApplication) error {
	reqLogger.Info("Adding Finalizer for WebsphereTraditionalApplication")
	wtapp.SetFinalizers(append(wtapp.GetFinalizers(), applicationFinalizer))

	// Update CR
	err := r.GetClient().Update(context.TODO(), wtapp)
	if err != nil {
		reqLogger.Error(err, "Failed to update WebsphereTraditionalApplication with finalizer")
		return err
	}
...
func (r *ReconcileWebsphereTraditional) deletePVC(reqLogger logr.Logger, pvcName string, pvcNamespace string) {
